...
Автотесты для данного проекта написаны на Java с использованием фреймворка <code>Selenoid</code>. При проектировании тестов применён паттерн <code>PageObject</code>. Для части автотестов проекта предусловие выполняется через запросы по API с помощью <code>REST-assured</code>.
